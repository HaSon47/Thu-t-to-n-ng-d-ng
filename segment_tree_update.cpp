...
//update Tree
void update_Tree(int i, int v, int l, int r, int pos){
    if (l==r && l==i) {seg_tree[pos]=v; return;}
    int mid = (l+r)/2;
    if (i<=mid){
        update_Tree(i,v,l,mid,2*pos);
    }else{
        update_Tree(i,v,mid+1,r,2*pos+1);
    }
    seg_tree[pos]= min(seg_tree[2*pos],seg_tree[2*pos+1]);
}
...
int main()
{
    ...
    int m; cin>>m;
    string s; int tmp1, tmp2;
    for(int i=0;i<m;i++){
        cin>>s>>tmp1>>tmp2;
        if(s=="get-max"){
            cout<<RMQ(tmp1,tmp2,1,n,1)<<endl;
        }
        if(s=="update"){
            update_Tree(tmp1,tmp2,1,n,1);
        }
    }
    return 0;
}
